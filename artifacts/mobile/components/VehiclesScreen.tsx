...
import {
  View, Text, TouchableOpacity, TextInput, ScrollView, Modal, SafeAreaView,
} from "react-native";
...
export default function VehiclesScreen({ visible, onClose }: Props) {
      ...
      <View style={{ flex: 1, backgroundColor: C.background }}>
        <SafeAreaView style={{ flex: 1 }}>
          <ScrollView contentContainerStyle={{ paddingHorizontal: S.xl, paddingBottom: S.xxxl + 20 }}>
            ...
            {filtered.length > 0 ? (
              filtered.map(v => (
                <TouchableOpacity key={v.id} onPress={onClose} activeOpacity={0.8}

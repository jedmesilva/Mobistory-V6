...
const IDENTITY = {
  ...
  historico: [
    { version: 3, date: "12 abr. 2026", desc: "Cor alterada para Azul Metálico" },
    { version: 2, date: "03 ago. 2024", desc: "Acessório adicionado: Engate reboque" },
    { version: 1, date: "15 jan. 2023", desc: "Identidade criada" },
  ],
};
  ...
  return (
    <div style={{ position: "fixed", inset: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: colors.background, zIndex: 60, overflowY: "auto", animation: "fadeIn .2s" }}>
      <div style={{ padding: `${spacing.xxxl}px ${spacing.xl}px ${spacing.xxxl}px` }}>
        ...
        <div style={{ background: colors.surface, borderRadius: radii.xxl, padding: spacing.xl, marginBottom: spacing.md }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <div>
              ...
              <div style={{ fontSize: font.size.xs, color: colors.textTertiary, marginTop: 2 }}>por {IDENTITY.emittedBy}</div>
            </div>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: font.size.xs, color: colors.textTertiary, fontWeight: font.weight.medium, marginBottom: spacing.xs }}>Versão</div>
              <div style={{ fontSize: font.size.sm, fontWeight: font.weight.bold, color: colors.textPrimary }}>v{IDENTITY.version}</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: font.size.xs, color: colors.textTertiary, fontWeight: font.weight.medium, marginBottom: spacing.xs }}>Última verificação</div>
              <div style={{ fontSize: font.size.sm, fontWeight: font.weight.semibold, color: colors.textPrimary }}>{IDENTITY.lastVerified}</div>
              <div style={{ fontSize: font.size.xs, color: colors.textTertiary, marginTop: 2 }}>Próxima: {IDENTITY.nextVerified}</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ position: "relative" }}>
          <div style={{ position: "absolute", left: 19, top: 20, bottom: 20, width: 1, background: colors.border }} />
          {IDENTITY.historico.map(({ version, date, desc }, i) => (
            <div key={version} style={{ display: "flex", gap: spacing.lg, marginBottom: i < IDENTITY.historico.length - 1 ? spacing.xl : 0, position: "relative" }}>
              <div style={{ width: 40, height: 40, borderRadius: "50%", background: version === IDENTITY.version ? colors.accent : colors.surface, border: `1px solid ${version === IDENTITY.version ? colors.accent : colors.border}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, zIndex: 1 }}>
                <span style={{ fontSize: font.size.xxs, fontWeight: font.weight.bold, color: version === IDENTITY.version ? colors.textInverse : colors.textTertiary }}>v{version}</span>
              </div>
              <div style={{ flex: 1, paddingTop: spacing.xs }}>
                <div style={{ display: "flex", alignItems: "center", gap: spacing.sm }}>
                  <span style={{ fontSize: font.size.base, fontWeight: font.weight.semibold, color: colors.textPrimary }}>
                    {version === 1 ? "Identidade emitida" : "Identidade atualizada"}
                  </span>
                  {version === IDENTITY.version && (
                    <span style={{ fontSize: font.size.xxs, fontWeight: font.weight.semibold, color: colors.textInverse, background: colors.accent, borderRadius: radii.pill, padding: `2px ${spacing.sm}px` }}>Atual</span>
                  )}
                </div>
                <div style={{ fontSize: font.size.sm, color: colors.textSecondary, marginTop: 2 }}>{desc}</div>
                <div style={{ fontSize: font.size.xs, color: colors.textTertiary, marginTop: spacing.xs }}>{date}</div>
